...
TR = {
    "he": {
        ...
        "feat3":            "דוח מסכם מיידי",
        "feat3_desc":       "המלצה ברורה — לקנות או לא",
    },
    "en": {
        ...
        "feat3":            "Instant Summary Report",
        "feat3_desc":       "Clear GO / NO GO recommendation",
    }
}
...
def section_label(key: str):
    st.markdown(
        f"<p style='font-size:0.7rem;color:var(--muted);letter-spacing:0.2em;text-transform:uppercase;margin-bottom:0.3rem;{rtl_css}'>{t(key)}</p>",
        unsafe_allow_html=True,
    )
...
def render_result(result: dict):
    ...
    <div style='background:{bg};border:1px solid {color};border-radius:6px;
        ...
        <div style='font-size:0.78rem;letter-spacing:0.2em;color:var(--muted);
                    text-transform:uppercase;'>{car_label}</div>
        <div style='font-size:0.65rem;color:var(--muted);margin-top:0.3rem;'>{date}</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if steps:
        ...
        for i, s in enumerate(steps, 1):
            ...
            <div style='display:flex;align-items:flex-start;gap:0.75rem;margin:0.5rem 0;{rtl_css}'>
                ...
                <div style='font-size:0.85rem;color:var(--text);padding-top:0.2rem;'>{s.get("text","")}</div>
            </div>
            """, unsafe_allow_html=True)
    ...
    for i, (col, name, icon) in enumerate(zip(cols, labels, STEP_ICONS), 1):
        with col:
            ...
            <div style='text-align:center;'>
                ...
                <div style='font-size:0.68rem;letter-spacing:0.1em;text-transform:uppercase;color:{txt};'>{name}</div>
            </div>
            """, unsafe_allow_html=True)
    gold_divider()
...
# ─── Login screen ─────────────────────────────────────────────────────────────
def login_screen():
    lang_toggle(key_prefix="login_")

    # ── Hero banner with car image ───────────────────────────────────────────
    st.markdown(f"""
    <div style="
        background:
            linear-gradient(to bottom, rgba(12,12,12,0.25) 0%, rgba(12,12,12,0.75) 55%, rgba(12,12,12,1) 100%),
        ...
        text-align: center;
    ">
        <div style='font-family:Cormorant Garamond,serif;font-weight:300;font-size:3.4rem;
        ...
        <div style='font-size:0.72rem;letter-spacing:0.4em;color:rgba(240,235,224,0.65);
    ...
    with col_feat:
        ...
        for feat_key, desc_key, icon in [
            ...
            st.markdown(f"""
            <div class='feat-card'>
                <div style='display:flex;align-items:center;gap:0.75rem;{rtl_css}'>
                    <div style='font-size:1.4rem;'>{icon}</div>
                    <div>
                        <div style='font-size:0.85rem;color:var(--gold);font-weight:500;'>{t(feat_key)}</div>
                        <div style='font-size:0.75rem;color:var(--muted);margin-top:0.15rem;'>{t(desc_key)}</div>
                    </div>
                </div>
            </div>
            """, unsafe_allow_html=True)
    ...
    with col_form:
        ...
        st.markdown(
            f"<p style='font-size:0.65rem;color:var(--muted);text-align:center;margin-top:0.8rem;'>{t('email_hint')}</p>",
            unsafe_allow_html=True,
        )
        st.markdown("</div>", unsafe_allow_html=True)
...
def render_sidebar():
    with st.sidebar:
        ...
        <div style='padding:0.8rem 0 0.4rem;{rtl_css}'>
            ...
            <div style='font-size:0.62rem;color:var(--muted);letter-spacing:0.2em;
                        text-transform:uppercase;margin-top:0.15rem;'>{t("app_subtitle")}</div>
        </div>
        """, unsafe_allow_html=True)

        st.markdown(f"<p style='font-size:0.72rem;color:var(--muted);margin:0.2rem 0 0.6rem;{rtl_css}'>{st.session_state.email}</p>", unsafe_allow_html=True)
        gold_divider()
        ...
        if past:
            st.markdown(f"<p style='font-size:0.65rem;color:var(--muted);letter-spacing:0.2em;text-transform:uppercase;margin-top:1.2rem;{rtl_css}'>{t('past_checks')}</p>", unsafe_allow_html=True)
            for check in past[:15]:
                ...
                st.markdown(f"<div style='font-size:0.7rem;color:{v_color};margin-bottom:0.1rem;{rtl_css}'>{v_label}</div>", unsafe_allow_html=True)
...
def step_vehicle_details():
    ...
    if st.button(t("continue_btn")):
        st.session_state.car_details = {
            ...
            "year": int(year), "trim": trim.strip(),
            "odometer": int(odometer), "nickname": nickname.strip(),
        }
...
def step_photos():
    ...
    st.markdown(f"<p style='font-size:0.83rem;color:var(--muted);{rtl_css}'>{t('photos_hint')}</p>", unsafe_allow_html=True)
    ...
    st.markdown(f"<p style='font-size:0.83rem;color:var(--muted);{rtl_css}'>{t('underbody_hint')}</p>", unsafe_allow_html=True)
...
def step_audio():
    ...
    st.markdown(f"<p style='font-size:0.83rem;color:var(--muted);{rtl_css}'>{t('audio_hint')}</p>", unsafe_allow_html=True)
...
def main_app():
    ...
    <div style='text-align:center;padding:1.2rem 0 0.6rem;'>
        ...
        <div style='font-size:0.65rem;letter-spacing:0.3em;color:var(--muted);text-transform:uppercase;margin-top:0.2rem;'>
            {t("app_subtitle_main")}
        </div>
    </div>
    """, unsafe_allow_html=True)
