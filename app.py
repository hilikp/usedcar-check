...
TR = {
    "he": {
        ...
        "underbody_title":  "תמונת תחתית הרכב",
    ...
    "en": {
        ...
        "underbody_title":  "Underbody Photo",
...
# Writes are fire-and-forget background threads — local JSON remains the read
# source of truth so a Supabase outage never breaks the app.
#
# Required Streamlit secrets:
#   [supabase]
#   url = "https://xxxx.supabase.co"
#   key = "eyJ..."          ← anon/public key from Settings → API

_SB_CLIENT = None   # supabase client, initialised once

def _sb_init():
    """Initialise Supabase client once. Returns client or None."""
    global _SB_CLIENT
    if _SB_CLIENT is not None:
        return _SB_CLIENT
    try:
        _cfg = st.secrets.get("supabase", {})
        if not _cfg or not _cfg.get("url") or not _cfg.get("key"):
            return None
        from supabase import create_client
        _SB_CLIENT = create_client(_cfg["url"], _cfg["key"])
        return _SB_CLIENT
    except Exception:
        return None
...
def _sb_insert_check(check_id: str, email: str, result: dict):
    """Insert one row into the checks_log table."""
    sb = _sb_init()
    if not sb:
        return
    try:
        d = result.get("car_details", {})
        sb.table("checks_log").insert({
            "check_id":    check_id,
            "email":       email,
            "car_label":   result.get("car_label", ""),
            "verdict":     result.get("recommendation", ""),
            "confidence":  result.get("confidence", ""),
            "odometer_km": str(d.get("odometer", "")),
            "year":        str(d.get("year", "")),
...
def _sb_fire(fn, *args, **kwargs):
    """Run a Supabase write in a background thread (non-blocking)."""
    import threading
...
def get_or_create_user(email: str):
    ...
    if email not in users:
        ...
        _sb_fire(_sb_upsert_user, email, first_seen, 0)
    return users[email]
...
def save_check(email: str, result: dict) -> str:
    ...
    save_users(users)
    # Update user check_count + append check row in Supabase (background)
    _check_count = len(users[email]["checks"])
    _first_seen  = users[email].get("created_at", result["created_at"])
    _sb_fire(_sb_upsert_user,   email, _first_seen, _check_count)
    _sb_fire(_sb_insert_check,  check_id, email, result)
    return check_id
...
def render_result(result: dict):
    ...
    def _pdf_sec(title, body):
        if not body: return ""
        return f"<h2 style='font-size:13px;margin-top:14px;padding-bottom:3px;border-bottom:1px solid #4ad493;color:#333;'>{title}</h2>{body}"
...
<style>
  ...
  .header {{ border-bottom: 2px solid #4ad493; padding-bottom: 8px; margin-bottom: 10px; }}
  .app-name {{ font-size: 10px; color: #4ad493; letter-spacing: 2px; text-transform: uppercase; }}
...
<div class="footer">UsedCar Check &mdash; usedcar-check-if-the-car-is-worth-it.streamlit.app</div>
    ...
    # ── Generate PDF using reportlab + DejaVu font (Hebrew support) ──────────
    def _build_pdf() -> bytes:
        from io import BytesIO
        import re as _re_pdf, os as _os, unicodedata as _ud
