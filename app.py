...
/* ── Hide Streamlit "Press Enter to apply" hint that overlaps placeholder ── */
.stTextInput small,
.stNumberInput small,
...
def run_analysis(car_details, photo_files, audio_file, underbody_file=None, video_file=None) -> tuple:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        return decision, audio_dur, ai_report, nhtsa_data, audio_metrics, audio_findings_raw
        ...
        if st.button(t("analyse_btn")):
            if not audio:
                ...
                with st.spinner(t("analysing")):
                    try:
                        decision, audio_dur, ai_report, nhtsa_data, audio_metrics, audio_findings_raw = run_analysis(
